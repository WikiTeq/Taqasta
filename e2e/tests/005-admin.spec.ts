...
test('can log in to existing account', async ({page}) => {
    ...
    // Username
    await expect(page.locator('#wpName1')).toHaveCount(1);
    await page.locator('#wpName1').fill( ADMIN_USER_NAME );

    // Password
    await expect(page.locator('#wpPassword1')).toHaveCount(1);
    await page.locator('#wpPassword1').fill( ADMIN_USER_PASS );

    // Submit the form
    await expect(page.locator('#wpLoginAttempt')).toHaveCount(1);
...
test('can grant user rights', async ({page}) => {
    ...
    await page.goto('/wiki/Special:UserRights/PlaywrightTester');

    // Admin
    await expect(page.locator('#wpGroup-sysop')).toHaveCount(1);
    await page.locator('#wpGroup-sysop').check();

    // Reason
    await expect(page.locator('#wpReason')).toHaveCount(1);
    await page.locator('#wpReason').fill( 'Grant adminship' );

    // Submit the form
    await expect(page.locator('input[name="saveusergroups"]')).toHaveCount(1);
...
test('can delete pages', async ({page}) => {
    // Need to log in
    await page.goto('/wiki/Special:UserLogin');
    await page.locator('#wpName1').fill( 'PlaywrightTester' );
    ...
    await expect(page.locator('#p-cactions-checkbox')).toHaveCount(1);
    await page.locator('#p-cactions-checkbox').check();
    await expect(page.locator('#ca-delete > a')).toHaveCount(1);
    await page.locator('#ca-delete > a').click();

    // Deletion form, use default reason
    await expect(page).toHaveTitle(/Delete "Deletion test"/);
    await expect(page.locator('button[name="wpConfirmB"]')).toHaveCount(1);
    ...
    await expect(page.locator('.noarticletext')).toHaveCount(1);
